...
4. **Settings → Environment Variables**: add `INSTANTLY_API_KEY`, `APP_USER`, `APP_PASS`.
5. Deploy. Done.
...
- **Webhooks instead of polling:** Instantly can push `campaign_completed`-type events to an endpoint — add `app/api/instantly/webhook/route.ts`, verify a shared secret, and update Redis on receipt. (Webhooks require a higher Instantly tier.)
...
app/api/instantly/sync/route.ts— Instantly proxy
